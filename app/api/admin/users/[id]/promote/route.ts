...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  const pg = getPgPool();
  await pg.query('UPDATE "user" SET is_admin=true WHERE id=$1', [params.id]);
  return new Response(JSON.stringify({ message: 'User promoted' }), { headers: { 'Content-Type': 'application/json' } });
}
